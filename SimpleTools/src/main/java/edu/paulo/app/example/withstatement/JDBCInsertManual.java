...
public class JDBCInsertManual {
	...
	public JDBCInsertManual(String[][] strDatas , String tableName)
	{
	    exceptionString[0] = "JDBCInsertManual";
		cProp = new ConfigProperties();
		stdb = new SimpleToolsDB();
		setData(strDatas, tableName);
	}
	
    public static void main(String[] args) {
		
    	
    	String[][] strData = new String[2][3];
    	strData[0][0] = "id";
    	strData[0][1] = "first_name";
    	strData[0][2] = "last_name";
		
		strData[1][0] = "1";
		strData[1][1] = "Mafasyapa";
		strData[1][2] = "Annisa";
		
		/*DATA UNTUK DEMO*/
		JDBCInsertManual jife = new JDBCInsertManual(strData, "insert_demo");		
   }
	
	public void setData(String[][] strDatas , String tableName)
	{
		exceptionString[1] = "setData(String[][] strDatas , String tableName)";
		/*SET DRIVER BY PARAMETER*/
		try {
            conn = stdb.getDatabaseConnection();
		    System.out.println("Connection is created successfully!!");
		    stmt = conn.createStatement();			
		    execData(stmt, conn, strDatas ,tableName);
         } catch (Exception e) {
            stdb.exceptionStringz(exceptionString, e, cProp.getfException());
         }
	}
	
	public void execData(Statement stment, Connection connects,String[][] strDatas,String tabName)
	{
		  exceptionString[1] = "execData(Statement stment, Connection connects,String[][] strDatas,String tabName)";
	      try {
	    	  
	  	      intCol  = strDatas[0].length;//this is the column number
	  		  ...
	  		  {
	  		    sBuild.setLength(0);
	  			queryzHeader = sBuild.append(queryzHeader).append(strDatas[0][j]).toString();				
	  			if(j != intCol-1)
	  			{
	  				sBuild.setLength(0);
	  		  ...
	  		  queryzHeader = sBuild.append(queryzHeader).append(") VALUES ( ").toString();
	  			...
	  			for(int i=1;i<strDatas.length;i++)
	  			{
	  				...
	  				for(int j=0;j<intCol;j++)
	  				{
	  					...
	  					queryz = sBuild.append(queryz).append("'").append(strDatas[i][j].replace("'","")).append("'").
	  							append(",").toString();
	  				}
	  				sBuild.setLength(0);
